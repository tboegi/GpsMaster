...
public class ImageDisplay extends JComponent {
    ...
    private static final int DRAG_BUTTON = 1; // Main.pref.getBoolean("geoimage.agpifo-style-drag-and-zoom", false) ? 1 : 3;
    private static final int ZOOM_BUTTON = 3; // DRAG_BUTTON == 1 ? 3 : 1;
    ...
    public void paintComponent(Graphics g) {
        ...
        if (file == null) {
            ...
            String loadingStr;
            if (! errorLoading) {
                loadingStr = "Loading " + file.getName();
            } else {
                loadingStr = "Error on file " + file.getName();
            }
            ...
            if (errorLoading) {
                String loadingStr = "Error on file " + file.getName();
                Rectangle2D noImageSize = g.getFontMetrics(g.getFont()).getStringBounds(loadingStr, g);
                Dimension size = getSize();
                g.drawString(loadingStr,
                        (int) ((size.width - noImageSize.getWidth()) / 2),
